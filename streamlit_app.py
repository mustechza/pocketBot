...
def emoji(txt):
    return txt if USE_EMOJIS else ""

# -------------------- Config --------------------
st.set_page_config(layout="wide")
st.title(f"{emoji('📈')} Deriv Strategy Signal Dashboard")
...
def plot_strategy(df, strategy):
    ...
    if strategy == "EMA Crossover":
        ...
        fig.add_trace(go.Scatter(x=df.index, y=stoch_rsi, name='Stoch RSI'))
        ...
        df['Signal'] = np.where(df['HA_Close'] > df['HA_Open'], 'Buy',
                                np.where(df['HA_Close'] < df['HA_Open'], 'Sell', None))
        df['Confidence'] = (abs(df['HA_Close'] - df['HA_Open']) / df['close']) * 100

        fig.add_trace(go.Candlestick(x=df.index, open=df['HA_Open'], high=df['high'],
                                     low=df['low'], close=df['HA_Close'], name="Heikin-Ashi"))
        ...
        while True:
            try:
                ...
                if 'candles' in data:
                    ...
                    candle_df = df
                    return  # only fetch once per run
            except:
                continue
